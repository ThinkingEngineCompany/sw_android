...
import com.cgfay.camera.camera.CalculateType;
...
public class CameraControllerX implements ICameraController, Camera.PreviewCallback {
...
//        camera.setParameters(parameters);
        ...
        switch (calculateType) {
            ...
            case Min:
                ...
                if (noBigEnough.size() > 1) {
                    perfectSize = Collections.min(noBigEnough, new CameraController.CompareAreaSize());
            ...
            case Max:
                ...
                // 因此，当只有一个元素时，直接使用该元素
                if (bigEnough.size() > 1) {
                    perfectSize = Collections.max(bigEnough, new CameraController.CompareAreaSize());
            ...
            case Lower:
                ...
                if (noBigEnough.size() > 0) {
                    Camera.Size size = Collections.max(noBigEnough, new CameraController.CompareAreaSize());
                    if (((float) size.width / expectWidth) >= 0.8
                            && ((float) size.height / expectHeight) > 0.8) {
                        perfectSize = size;
                    }
                } else if (bigEnough.size() > 0) {
                    Camera.Size size = Collections.min(bigEnough, new CameraController.CompareAreaSize());
    ...
    private static void sortList(List<Camera.Size> list) {
        Collections.sort(list, new CameraController.CompareAreaSize());
    }
